...
@perf_time
def s4_find_median_sorted_arrays(nums1, nums2):
  """
...
@perf_time
def s14_longest_common_prefix(strs):
  if not strs:
    return ''
  # simple method
...
@perf_time
def s16_three_sum_closest(nums, target):
...
@perf_time
def s26_remove_duplicates(nums):
  if not nums:
    return 0
  pos = 0
...
@perf_time
def s27_remove_element(nums, val):
  pos = 0
  for n in nums:
    if n != val:
      nums[pos] = n
...
@perf_time
def s29_divide(dividend, divisor):
...
@perf_time
def s34_search_range(nums, target):
  """
  :tags: Binary Search
  :param nums:
  :param target:
...
@perf_time
def s35_search_insert(nums, target):
  """
  :tags: Binary Search
  :param nums:
  :param target:
